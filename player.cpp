...
// –ˆƒtƒŒ[ƒ€XViƒ_ƒbƒVƒ…Œp‘±ŽžŠÔ‚ÌŠÇ—A“ü—Íˆ—j
void Player::Update(double elapsedSec)
{
	// 1. ƒ_ƒbƒVƒ…ŽžŠÔ‚ÌXV
	if (isDashing_) {
...
<<<<<<< HEAD
	// HealthƒRƒ“ƒ|[ƒlƒ“ƒgÝ’è
	AddComponent<Health>(100.0f);

	// PlayerMovementƒRƒ“ƒ|[ƒlƒ“ƒgÝ’è
	AddComponent<PlayerMovement>(camcomp);

	// PlayerMorphSystemƒRƒ“ƒ|[ƒlƒ“ƒgÝ’è
	AddComponent<PlayerMorphSystem>();
=======
					//“d‹C‰»‚µ‚½uŠÔ‚Íis•ûŒü‚ð‰Šú‰»
					electricMoveDir_ = XMFLOAT3(0.0f, 0.0f, 0.0f);
				}
			} else {
				// ELECTRICITY -> HUMAN: “d’Œ‹ß‚­•K{
				if (IsNearPole()) {
					...
					// “d’Œ‚©‚ç’µ‚Ë•Ô‚·ˆ—
					KnockbackFromPole();
					// 0.5•bŠÔAÕ“Ë”»’è‚ðƒXƒLƒbƒv
					skipCollisionTimer_ = SKIP_COLLISION_DURATION;

					//lŠÔ‚É–ß‚éŽž‚àƒŠƒZƒbƒg
					electricMoveDir_ = XMFLOAT3(0.0f, 0.0f, 0.0f);
				}
			}
		}
>>>>>>> parent of 4f2a780 (inertiaã¨å°„å‡ºã‚¢ã‚¯ã‚·ãƒ§ãƒ³ã®è¿½åŠ (å®Œäº†))
	...
	// PlayerStateMachineƒRƒ“ƒ|[ƒlƒ“ƒgÝ’è
...
>>>>>>> parent of 4f2a780 (inertiaã¨å°„å‡ºã‚¢ã‚¯ã‚·ãƒ§ãƒ³ã®è¿½åŠ (å®Œäº†))
		...
		if (usePlayer == false)
		{
			...
			// ó‘Ô‚ðHUMAN‚ÉƒŠƒZƒbƒg
			ChangeState(State::HUMAN);
			
			// ˆÚ“®‘¬“x‚ðÝ’è
			currentSpeed_ = baseSpeed_;
			
			// ƒ_ƒbƒVƒ…ó‘Ô‚ðƒŠƒZƒbƒg
			isDashing_ = false;
			dashTimeRemaining_ = 0.0f;
	...
	// 2. d—Í‚ð“K—pi–ˆƒtƒŒ[ƒ€‘¬“x‚ð‰ÁŽZj
	if (!isGrounded_) {
	...
	// “d‹Có‘Ô‚Ìê‡‚ÍÕ“Ë”»’è‚ðƒXƒLƒbƒv‚µ‚Ä’¼i
	if (state == State::ELECTRICITY) {
		...
		// ó‘Ô•Ï‰»’¼Œã‚ÌƒXƒLƒbƒvƒ^ƒCƒ}[ŠúŠÔAÕ“Ë”»’è‚ðƒXƒLƒbƒv
		m_Position.x += desiredMove.x;
		m_Position.z += desiredMove.z;
		m_Position.y += desiredMove.y;
	} else {
		// HUMANó‘Ô‚Í’Êí‚ÌÕ“Ë”»’è‚ð“K—p
		ResolveCollisions(desiredMove);
	}

	// 4. “d‹Có‘Ô‚Ìê‡A“dü‚ÉƒXƒiƒbƒv‚³‚¹‚Ä—Ž‰º–hŽ~
	if (state == State::ELECTRICITY) {
...
void Player::ResolveCollisions(DirectX::XMFLOAT3& desiredMove/*, double elapsedSec*/)
{
    ...
    for (const auto& obj : g_ObjectManager.GetGameObjects()) {
        // HUMANó‘Ô‚ÌÛAPowerLine‚Æ‚ÌÕ“Ë‚ðƒXƒLƒbƒv
        if (state == State::HUMAN && obj->GetTag() == GameObjectTag::POWER_LINE) {
            continue;
        }
        // ƒvƒŒƒCƒ„[Ž©g‚ÍÕ“Ë”»’è‚©‚çœŠO
    ...
    if (!collisionXZ) {
        m_Position = newPos; // Õ“Ë‚È‚¯‚ê‚ÎˆÚ“®‚ð“K—p
    }

    // YŽ²i‚’¼j‚ÌˆÚ“®‚ÆÕ“Ë”»’è
    m_Position.y += desiredMove.y;
    playerAABB = GetAABB(); // XV‚³‚ê‚½XZˆÊ’u‚ÅAABB‚ðÄŒvŽZ
    ...
    for (const auto& obj : g_ObjectManager.GetGameObjects()) {
        ...
        if (playerAABB.IsOverlap(obj->GetAABB())) {
            // ƒIƒuƒWƒFƒNƒg‚Ìã‚É‚ ‚é‚©A‰º‚É‚ ‚é‚©‚Å•ª‚¯‚é
            if (desiredMove.y < 0.0f) { // —Ž‰ºŽž
                m_Position.y = obj->GetAABB().GetMax().y; // ƒIƒuƒWƒFƒNƒg‚Ìã–Ê‚ÉƒXƒiƒbƒv
                velocityY_ = 0.0f;
                isGrounded_ = true;
            } else if (desiredMove.y > 0.0f) { // ã¸Žž
                m_Position.y = obj->GetAABB().GetMin().y - aabbHalfSize.y * 2.0f; // ƒIƒuƒWƒFƒNƒg‚Ì‰º–Ê‚ÉƒXƒiƒbƒv
                velocityY_ = 0.0f;
            }
            collisionY = true;
            break;
...
void Player::SnapToNearestPowerLine()
{
	...
	// ‚·‚×‚Ä‚Ì“dü‚Ì’†‚©‚çÅ‚à‹ß‚¢ƒ|ƒCƒ“ƒg‚ð’T‚·
	const auto& allObjects = g_ObjectManager.GetGameObjects();
	for (const auto& obj : allObjects) {
		if (obj->GetTag() == GameObjectTag::POWER_LINE) {
			...
			// …•½‹——£‚ª”ÍˆÍ“à‚È‚çA‚’¼‹——£‚ÉŠÖ‚í‚ç‚¸ƒXƒiƒbƒv‘ÎÛ‚É‚·‚é
			// …•½‹——£: 2mˆÈ“àA‚’¼‹——£: 5mˆÈ“à
			if (horizontalDist <= POWER_LINE_SNAP_DISTANCE && verticalDist <= 5.0f) {
				// 3ŽŸŒ³‹——£‚ÅÅ‚à‹ß‚¢‚à‚Ì‚ð‘I‚Ô
...
// “d‹Có‘Ô‚©‚çlŠÔó‘Ô‚É•Ï‰»‚·‚éÛ‚É“d’Œ‚©‚ç’µ‚Ë•Ô‚·
void Player::KnockbackFromPole()
{
	...
	// Å‚à‹ß‚¢“d’Œ‚©‚çƒvƒŒƒCƒ„[‚Ö‚Ì•ûŒüƒxƒNƒgƒ‹‚ðŒvŽZ
...
void Player::ResetToElectricityState()
{
	...
	// ƒ_ƒ[ƒWƒ^ƒCƒ}[‚ðƒŠƒZƒbƒgF“d‹Có‘Ô‚É•Ï‚í‚Á‚½’¼Œã‚Íƒ_ƒ[ƒW‚ðŽó‚¯‚È‚¢‚æ‚¤‚É‚·‚é
	powerLineDamageTimer_ = POWERLINE_DAMAGE_INTERVAL;

	// ÅŠñ‚è‚Ì“dü‚ÉƒXƒiƒbƒv
	SnapToNearestPowerLine();
}

// ƒvƒŒƒCƒ„[ƒXƒe[ƒg•ÏXƒƒ\ƒbƒh
void Player::ChangeState(Player::State newState)
{
	if (state == newState) return; // ó‘Ô‚ª•Ï‚í‚ç‚È‚¢ê‡‚Íˆ—‚µ‚È‚¢

	state = newState;

	// HUMANó‘Ô‚É–ß‚éê‡‚Íƒ^ƒCƒ}[‚ðƒŠƒZƒbƒg
