...
// UI: ハウス状態表示（HouseUI版）
static HouseUI* g_houseUI = nullptr;
...
void Game_Initialize()
{
	...
	// 各区画に家と電柱を配置
	// 
	// マップ全体サイズ: 40x40 units (-20～20, -20～20)
	// 中央原点: (0, 0)
	// 各区画サイズ: 20x20 units
...
void Game_Update(double elapsed_time)
{
	...
	//デバッグカメラ
	g_debugCamera.Update(elapsed_time);
	...
	if (g_uiHouse) {
		...
		// HouseUIにも家リストを設定
		if (g_houseUI) {
			g_houseUI->SetHouses(houses);
			g_houseUI->Update(elapsed_time);
		}
