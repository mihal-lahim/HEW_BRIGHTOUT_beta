...
House::House(const DirectX::XMFLOAT3& pos, MODEL* model, float maxElectricity)
...
void House::Update(double elapsed)
{
    ...
    // 復旧判定
    m_isRepaired = (m_electricity >= m_maxElectricity);
}
...
AABB House::GetAABB() const
{
    XMFLOAT3 halfSize(HOUSE_SIZE * AABB_SCALE_FACTOR, HOUSE_SIZE * AABB_SCALE_FACTOR, HOUSE_SIZE * AABB_SCALE_FACTOR);
    return { m_Position, halfSize };
}

float House::GetDistanceToPlayer(const XMFLOAT3& playerPos) const
{
    XMVECTOR diff = XMLoadFloat3(&playerPos) - XMLoadFloat3(&m_Position);
...
{
    m_isRepaired = repaired;
    if (repaired) m_electricity = m_maxElectricity;
}
