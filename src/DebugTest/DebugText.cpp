...
==============================================================================*/
#include "include/DebugTest/DebugText.h"
...
namespace hal
{
	...
	{
		m_TextLines.back().strings.emplace_back(color); //追加文字列バッファ

		for(; *pText; pText++) {

			if (*pText == '\n') {
				m_TextLines.emplace_back(); // 改行
				m_TextLines.back().strings.emplace_back(color); // 新しい行の文字列領域
			}
			else if (*pText == '\r') {
				// キャリッジリターンは無視
			}
				...
				while (m_TextLines.back().characterCount %4 !=0) {
					if (m_MaxCharactersPerLine && m_TextLines.back().characterCount >= m_MaxCharactersPerLine) {
						m_TextLines.emplace_back(); // 改行
						// m_TextLines.back().strings.emplace_back(color); //ここでは空白挿入のみ
						break;
					}
		...
		while(m_MaxLine && m_TextLines.size() + last_line_count > m_MaxLine) {
			...
			m_CharacterCount -= remove_character_count; // 全体文字数を調整
			m_TextLines.pop_front(); //先頭行を削除
		}
	}
